...
from helper import writeToFile, deleteDataFromFile
...
def main():
    ...
    start = time.time()
    deleteDataFromFile("data/all.txt")
    writeToFile(
        "data/all.txt",
        "count, interp , mean interp, normalize, mean norm, mva, mean mva, mva_inter, mean_mva_inter",
    )
    while True:
        ...
        if (len(mean_list) >= WINDOW_SIZE) and (len(mean_list) % FPS == 0):
            ...
            mva_inter = moving_avg(interpolated, 6)
            ...
            meanHr5 = sum(heart_rates5) / len(heart_rates5)
            ...
            text5 = "HR5 mva_inter: {:.2f}, {:.2f}".format(hr5, meanHr5)

            # writeToFile("data/original.txt", "{}, {}".format(hr1, meanHr1))
            # writeToFile("data/interpolate.txt", "{}, {}".format(hr2, meanHr2))
            # writeToFile("data/normalize.txt", "{}, {}".format(hr3, meanHr3))
            # writeToFile("data/mva.txt", "{}, {}".format(hr4, meanHr4))
            writeToFile(
                "data/all.txt",
                "{}, {:.2f}, {:.2f}, {:.2f}, {:.2f}, {:.2f}, {:.2f}, {:.2f}, {:.2f}".format(
                    count, hr2, meanHr2, hr3, meanHr3, hr4, meanHr4, hr5, meanHr5
                ),
            )
